...
def run():
    ...
    if cal.loc[str(pd.Timestamp.today().date()), 'is_open']:
        ...
        print('下次任务将于明日19:00执行！')
        print('本次任务结束时间：%s' % str(pd.Timestamp.today()))
    else:
        print('今日未开盘，任务结束。下次将于明日19:00执行。')
        print('本次任务结束时间：%s' % str(pd.Timestamp.today()))
...
if __name__ == '__main__':
    ...
    scheduler.add_job(run, 'cron', hour='19', minute='00', timezone='Asia/Shanghai')
    print('程序启动！启动时间：%s' % str(pd.Timestamp.today()))
    print('系列程序将于每日19:00分自动开始运行，只在开盘日执行下载和选股。。。请等待。。。')
    '''
